...
main = getArgs >>= inputProcessing >>= lesson

processInput :: [String] -> m (Either String (Deck, Config))
processInput args = case maybeFiles of
    Nothing -> pure $ Left "Input error (no deck given)"
    Just files -> mapM doGetDeck files =>> \decks ->
  ...
  where
    opt = ("--" <>)
    deck = tail $ trailWhen (=== opt "decks") args
    hasOpt key ifyes ifnot =
      if opt key `elem` args then ifyes else ifnot
    flipDeck deck = Arr.first (fmap flipTuple) deck

doGetDeck :: FilePath -> m Deck
doGetDeck path = getContents path =>> toDeck

toDeck :: String -> Deck
toDeck = linesOf / pieces "??" / fmap (pieces "--" / packed)
  where
    pieces e = splitOn e
    packed = take 3 / 
